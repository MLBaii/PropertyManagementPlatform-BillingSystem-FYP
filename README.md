...
| **Shared** | Billing API + database | ASP.NET Core Web API + PostgreSQL (Supabase) | `backend/` |

## Repository structure

```
propertybill/
├── backend/     # Shared ASP.NET Core Web API + EF Core + PostgreSQL (Supabase) migrations
...
1. Clone the repo.
2. Configure the backend connection string to the shared Supabase-hosted PostgreSQL database (`backend/`).
